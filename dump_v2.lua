--[[ Dump v2 -- dump.lua with a focus knob: search the tree, don't empty it.

     Same walk, same outputs, same zero per-map knowledge as dump.lua. The one
     difference is FOCUS: a lua pattern that decides which rows reach manifest.txt
     and tp.txt, and -- the part that actually costs minutes -- which scripts get
     decompiled. Everything else is byte-for-byte the old script.

     Three knobs, coarse to fine. Reach for them in this order:

         ROOTS         which services to walk at all. Dropping "Workspace" is the
                       big one: it's where the hundred thousand parts live, and
                       ReplicatedStorage alone gets you remotes.txt in seconds.
         FOCUS         pattern gating manifest/tp rows and decompiles.
         DO_PLACE      saveinstance freezes the client for a minute. Off by default
                       here; the whole point of a focused run is not waiting for it.

     FOCUS matches against the manifest row itself -- "ClassName<tab>Full.Path<tab>
     pivot<tab>attrs" -- so the pattern reaches every column:

         FOCUS = "Knit"              everything under a Knit folder
         FOCUS = "ProximityPrompt"   by class
         FOCUS = "Rarity=OG"         by attribute, the thing collect loops filter on
         FOCUS = "^Players%."        one subtree
         FOCUS = ""                  everything -- plain dump.lua behaviour

     Case-sensitive, because Roblox names are PascalCase and lowercasing a lua
     pattern breaks its character classes.

     FOCUS = "" is still the right first run on a new game. A filter can only find
     what you already knew to ask for; the unfiltered manifest is how you learn the
     game's vocabulary in the first place. Skim it, pick a word, re-run focused.

     remotes.txt and classes.txt ignore FOCUS on purpose. Remotes are a few hundred
     lines and are the file you end up scripting against -- a filtered one hides the
     remote you didn't know to look for. classes.txt is the census you pick your
     FOCUS from, so filtering it would defeat itself.
     ...
     Output, under the executor's workspace folder:
         ...
         dump/<PlaceId>/manifest.txt    matching instances: class, path, pivot, facts
         dump/<PlaceId>/remotes.txt     every remote, one per line
...
-- config ---------------------------------------------------------------------
local FOCUS = "" -- lua pattern; "" walks and writes everything, as dump.lua does
local DO_PLACE = false -- saveinstance the whole place; a minute-long client freeze
local DO_SCRIPTS = true -- decompile matching scripts to their own file
...
-- A typo'd pattern ("Rarity=(OG") would otherwise surface as an error thrown from
-- inside the walk, ten thousand instances in, looking like a bug in the walk.
assert(pcall(string.find, "", FOCUS), "FOCUS is not a valid lua pattern: " .. FOCUS)
...
-- focus ----------------------------------------------------------------------
-- Split from FOCUS so it takes the pattern as an argument and can be checked here
-- without the config deciding whether the check passes.
local function matches(pat, s)
	return pat == "" or s:find(pat) ~= nil
end

assert(matches("", "anything"), "empty pattern must pass everything")
assert(matches("Knit", "Folder\tgame.ReplicatedStorage.Knit.Services"), "substring match broken")
assert(not matches("Knit", "Folder\tgame.ReplicatedStorage.Net"), "non-match must be rejected")
assert(matches("Rarity=OG", "Model\tgame.Workspace.Br\t\tRarity=OG"), "attribute column must be matchable")

local function focused(s)
	return matches(FOCUS, s)
end
...
print(string.format(
	"[dump] place %d, %s | %d roots queued | focus=%s place=%s scripts=%s pivots=%s coregui=%s",
	game.PlaceId,
	identifyexecutor and identifyexecutor() or "unknown executor", -- and/or drops the version
	#stack,
	FOCUS == "" and "(all)" or ("%q"):format(FOCUS),
	tostring(DO_PLACE),
	tostring(DO_SCRIPTS),
	tostring(DO_PIVOTS),
	tostring(DO_COREGUI)
...
local count, scriptCount, failCount, skipCount = 0, 0, 0, 0
...
while #stack > 0 do
	...
	if count % YIELD_EVERY == 0 then
		...
		tick("walking: %d seen, %d kept, %d queued, %d scripts", count, #manifest, #stack, scriptCount)
	end
	...
	-- The walk itself always descends everything -- FOCUS filters what gets written,
	-- never where we go. Pruning the walk at a non-matching node would hide every
	-- match underneath it, and the interesting thing is usually three folders deep
	-- under a folder named something forgettable.
	if DO_PIVOTS then
		local pivot = ""
		...
		local row = table.concat({ inst.ClassName, full, pivot, extra(inst) }, "\t")
	...
	if DO_SCRIPTS and inst:IsA("LuaSourceContainer") then
		-- The whole reason FOCUS exists. decompile is seconds per script and the walk
		-- is milliseconds; a run that skips nine hundred of them finishes in a minute
		-- instead of an hour. Matched on the path alone -- there's no manifest row for
		-- a script beyond what its path already says.
		if not focused(inst.ClassName .. "\t" .. inst:GetFullName()) then
			skipCount += 1
		else
			local src, readable
...
if DO_PIVOTS then
	...
	-- An empty manifest reads as a broken dump. It's almost always the pattern: a
	-- lua pattern, not a glob, so "Zone*" and "*.Knit" match nothing at all.
	if #manifest == 0 and FOCUS ~= "" then
		warn(("[dump] FOCUS %q matched 0 of %d instances -- lua patterns, not globs: use %q, not %q")
			:format(FOCUS, count, "Zone", "Zone*"))
	end
end
...
-- Unfiltered on purpose: this is the file you pick a FOCUS out of.
local classN = 0
do
	local rows = {}
...
print(string.format(
	"[dump] done in %.1fs | %d instances, %d kept | %d classes | %d scripts (%d unreadable, %d skipped by focus) | %d remotes | %d tp points -> %s",
	os.clock() - t0,
	count,
	#manifest,
	classN,
